...
router.post("/creer", verifyToken, async (req, res) => {
  try {
    ...
    db.one(
      "INSERT INTO portfolio(user_id, titre, contenu, formation, experience, projet, langue) VALUES(${user_id}, ${titre}, ${contenu}, ${formation}, ${experience}, ${projet}, ${langue}) RETURNING id",
      {
        user_id: user_id,
        titre: "Votre titre",
...
router.get("/user/", verifyToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Les portfolios n'ont pas pu être récupérés" });
  }
});
...
router.put("/:id", verifyToken, async (req, res) => {
  try {
    const id = req.params.id;
    const { titre, contenu, public, formation, experience, projet, langue } =
      req.body;
    ...
    db.none(
      "UPDATE portfolio SET titre = ${titre}, contenu = ${contenu}, public = ${public}, formation = ${formation}, experience = ${experience}, projet = ${projet}, langue = ${langue} WHERE id = ${id}",
      {
        titre: titre,
        contenu: contenu,
        id: id,
...
router.delete("/:id", verifyToken, async (req, res) => {
  try {
    ...
    db.none("DELETE FROM portfolio WHERE id = $1", id).catch((error) => {
      console.log("ERROR:", error);
    });
